        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-center py-2">
            <div className="flex items-center space-x-2 text-sm font-medium text-primary">
              <span>
                {currentLang === "en"
                  ? "All items available at our Amsterdam store"
                  : "Alle items verkrijgbaar in onze winkel in Amsterdam"}
              </span>
            </div>
          </div>
        </div>
          ...
          <div className="flex justify-between items-center h-14 sm:h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              {navigation.map((item) => {
                ...
                if (
                  item.name ===
                  (currentLang === "en" ? "Categories" : "Categorieën")
                ) {
                  return (
                    <div
                      key={item.name}
                    ...
                    >
                      <Link
                        href={item.href}
                        className="font-sans text-sm font-medium text-foreground hover:text-primary transition-all duration-200 relative group hover:scale-105 py-2"
                      >
                        {item.name}
                      ...
                      {isCategoriesDropdownOpen && categories.length > 0 && (
                        <div
                          className={`
                          absolute top-full left-1/2 transform -translate-x-1/2 pt-2 z-50
                          transition-all duration-200 ease-out
                          ${isCategoriesDropdownOpen ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-95 -translate-y-2"}
                        `}
                        >
                          {/* Invisible bridge to maintain hover */}
                          <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-full h-2" />

                          <div className="bg-background rounded-lg shadow-lg border border-border py-4 w-96">
                            <div className="grid grid-cols-2 gap-2 px-4">
                              {categories.map((category, index) => (
                                <Link
                                  ...
                                  className={`
                                  flex items-center space-x-3 px-3 py-2 text-sm text-foreground
                                  hover:bg-secondary hover:text-primary rounded-md hover:translate-x-1 transition-all duration-150
                                  ${isCategoriesDropdownOpen ? "translate-y-0 opacity-100" : "translate-y-2 opacity-0"}
                                `}
                                  style={{
                                    transitionDelay: isCategoriesDropdownOpen
                                      ? `${index * 30}ms`
                                      : "0ms",
                                  }}
                                >
                ...
                {isLangDropdownOpen && (
                  ...
                  >
                    {languages.map((language, index) => (
                      ...
                      `}
                        style={{
                          transitionDelay: isLangDropdownOpen
                            ? `${index * 30}ms`
                            : "0ms",
                        }}
